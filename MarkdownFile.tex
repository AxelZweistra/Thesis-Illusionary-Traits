...
\authorsnames{Axel Zweistra,Jeroen Mulder,Ellen Hamaker}
...
\leftheader{Zweistra, Mulder and Hamaker}
...
\authornote{\par{\addORCIDlink{Axel
Zweistra}{0009-0000-8790-4608}}\par{\addORCIDlink{Jeroen
Mulder}{0000-0002-5553-0856}} 

\par{       }
\par{Correspondence concerning this article should be addressed to Axel
Zweistra, Department of Methods and Statistics, Faculty of Social and
Behavioural Sciences, Utrecht University, Padualaan 14, Utrecht 3584
CH, The
...
Bailey et al. (\citeproc{ref-baileyIllusoryTraitsWrong2024}{Bailey et
al., 2024}) by considering the effect of more timepoints. In addition,
we also vary the number of time-varying confounders and the strength of
the autoregressive effects across all variables to illustrate the role
of temporal stability in the observed processes.

\subsection{Extending the Simulation by Bailey et
al.}\label{extending-the-simulation-by-bailey-et-al.}
...
generating mechanism can be found in Figure 1. The data generating
mechanism consists of a bivariate CLPM with the addition of systems of
time-varying unobserved confounding. The relationships between the
observed variables X and Y are expressed in the autoregressive effects
...
all confounders on X having a lagged effect of .01 on the confounders on
Y and vice versa. Time-specific residual variances were specified to be
.5. After specifying the model, we sample from the stationary covariance
matrix of a single timepoint (see equation 2.1.39 in
\citeproc{ref-lutkepohlNewIntroductionMultiple2007}{Lütkepohl, 2007})
following the procedure in the original paper. This approach has the
benefit of not requiring to start up the process and wait for it to
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{images/DGM.png}
...
Figure 2 shows the bias in the cross-lagged parameter from \(X\) to
\(Y\), together with the estimated variance of the random intercept~for
\(X\), for each combination of the number of confounders and
autoregressive effects. We choose to display only one of the
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{images/fig_sim1_2x2.png}
...
of the total timespan of the study. Figure 3 shows a schematic example
of a SRI-CLPM, specified on segments with two timepoints.
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{images/SRI-CLPM.png}
...
\section{TABLE1:TOEPLITZ}\label{table1toeplitz}
...
kinds of covariances.}{\label{tbl-ri-covariance}}}
\vspace{-20pt}}
...
\hline
\begingroup\fontsize{13}{15}\selectfont \textbf{ }\endgroup & \begingroup\fontsize{13}{15}\selectfont \textbf{RIx1}\endgroup & \begingroup\fontsize{13}{15}\selectfont \textbf{RIx2}\endgroup & \begingroup\fontsize{13}{15}\selectfont \textbf{RIx3}\endgroup & \begingroup\fontsize{13}{15}\selectfont \textbf{RIx4}\endgroup & \begingroup\fontsize{13}{15}\selectfont \textbf{RIy1}\endgroup & \begingroup\fontsize{13}{15}\selectfont \textbf{RIy2}\endgroup & \begingroup\fontsize{13}{15}\selectfont \textbf{RIy3}\endgroup & \begingroup\fontsize{13}{15}\selectfont \textbf{RIy4}\endgroup\\
\hline
RIx1 & \cellcolor{lightgrey}{$\psi_{x,x}$} & \cellcolor{lightgrey}{} & \cellcolor{lightgrey}{} & \cellcolor{lightgrey}{} & \cellcolor{white}{} & \cellcolor{white}{} & \cellcolor{white}{} & \cellcolor{white}{}\\
\hline
RIx2 & \cellcolor{lightgrey}{$\psi^{lag1}_{x,x}$} & \cellcolor{lightgrey}{$\psi_{x,x}$} & \cellcolor{lightgrey}{} & \cellcolor{lightgrey}{} & \cellcolor{white}{} & \cellcolor{white}{} & \cellcolor{white}{} & \cellcolor{white}{}\\
\hline
RIx3 & \cellcolor{lightgrey}{$\psi^{lag2}_{x,x}$} & \cellcolor{lightgrey}{$\psi^{lag1}_{x,x}$} & \cellcolor{lightgrey}{$\psi_{x,x}$} & \cellcolor{lightgrey}{} & \cellcolor{white}{} & \cellcolor{white}{} & \cellcolor{white}{} & \cellcolor{white}{}\\
\hline
RIx4 & \cellcolor{lightgrey}{$\psi^{lag3}_{x,x}$} & \cellcolor{lightgrey}{$\psi^{lag2}_{x,x}$} & \cellcolor{lightgrey}{$\psi^{lag1}_{x,x}$} & \cellcolor{lightgrey}{$\psi_{x,x}$} & \cellcolor{white}{} & \cellcolor{white}{} & \cellcolor{white}{} & \cellcolor{white}{}\\
\hline
RIy1 & \cellcolor{darkgrey}{$\psi^{lag0}_{x,y}$} & \cellcolor{darkgrey}{$\psi^{lag1}_{x,y}$} & \cellcolor{darkgrey}{$\psi^{lag2}_{x,y}$} & \cellcolor{darkgrey}{$\psi^{lag3}_{x,y}$} & \cellcolor{grey}{$\psi_{y,y}$} & \cellcolor{grey}{} & \cellcolor{grey}{} & \cellcolor{grey}{}\\
\hline
RIy2 & \cellcolor{darkgrey}{$\psi^{lag1}_{x,y}$} & \cellcolor{darkgrey}{$\psi^{lag0}_{x,y}$} & \cellcolor{darkgrey}{$\psi^{lag1}_{x,y}$} & \cellcolor{darkgrey}{$\psi^{lag2}_{x,y}$} & \cellcolor{grey}{$\psi^{lag1}_{y,y}$} & \cellcolor{grey}{$\psi_{y,y}$} & \cellcolor{grey}{} & \cellcolor{grey}{}\\
\hline
RIy3 & \cellcolor{darkgrey}{$\psi^{lag2}_{x,y}$} & \cellcolor{darkgrey}{$\psi^{lag1}_{x,y}$} & \cellcolor{darkgrey}{$\psi^{lag0}_{x,y}$} & \cellcolor{darkgrey}{$\psi^{lag1}_{x,y}$} & \cellcolor{grey}{$\psi^{lag2}_{y,y}$} & \cellcolor{grey}{$\psi^{lag1}_{y,y}$} & \cellcolor{grey}{$\psi_{y,y}$} & \cellcolor{grey}{}\\
\hline
RIy4 & \cellcolor{darkgrey}{$\psi^{lag3}_{x,y}$} & \cellcolor{darkgrey}{$\psi^{lag2}_{x,y}$} & \cellcolor{darkgrey}{$\psi^{lag1}_{x,y}$} & \cellcolor{darkgrey}{$\psi^{lag0}_{x,y}$} & \cellcolor{grey}{$\psi^{lag3}_{y,y}$} & \cellcolor{grey}{$\psi^{lag2}_{y,y}$} & \cellcolor{grey}{$\psi^{lag1}_{y,y}$} & \cellcolor{grey}{$\psi_{y,y}$}\\
\hline
\end{tabular}
\endgroup{}
...
Figure 3 shows the distributions of one of the cross-lagged parameter
estimates, visualized as boxplots, as they are obtained from various
models and for an increasing number of timepoints. The true parameter
value is .1, displayed as the horizontal dotted line. The RI-CLPM's
cross-lagged parameter estimates slowly increase to the uncorrected
biased estimates retrieved by the CLPM. As expected, the SRI-CLPM is
able to retrieve less biased cross-lagged parameter estimates when the
number of timepoints considered increases, at the cost of somewhat
larger variance in the sampling distribution compared to the CLPM and
RI-CLPM. Figure 3 also shows the effect of the segment length chosen for
the random intercepts has on the bias and sampling variability of the
cross-lagged parameters. Increasing the segment length from two to three
leads to a slight increase in bias, but in turn also reduces sampling
variability slightly, which is likely due to the decrease in the number
of parameters estimated that needs to be estimated when the segment is
larger.~
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{images/fig_T_effect_bx.png}

\end{figure}%

Figure 4 shows the 95\% confidence interval coverage rates for all
structural parameters (also including the autoregressive parameters),
comparing the SRI-CLPM (right column) with Toeplitz latent variable
constraints and a segment length of 2 to the CLPM (left column) and
RI-CLPM (middle column). The coverage rates for cross-lagged parameters
show a substantial improvement over the biased estimates in the RI-CLPM
and CLPM. Interestingly however, the autoregressive parameters show a
marked decline in 95\% CI coverage rates as more timepoints are
considered. Another puzzling aspect of this is that it seems the model
decline is more pronounced on the larger sample size, which is somewhat
alarming. It is important to note however, that retrieving unbiased
estimates of autoregressive parameters was not the goal of this
simulation and is usually not the goal of cross-lagged panel modeling.
...
presented in Figure 4 below, along with the statistical power and type I
error. The SRI-CLPM shows acceptable power on samples as small as 100
cases and a relatively uniform p-value distribution on the cross-lagged
effect that was set to zero. There is however a somewhat inflated type I
error rate when compared to the nominal \(\alpha\) of .05 used, which
importantly does decrease on larger sample sizes.
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{images/fig_power.png}
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{images/SGRI-CLPM.png}
...
each of the conditions are plotted in Figure 3. Interestingly, the model
specifications seem to perform identically at retrieving unbiased
cross-lagged parameter estimates, even when a time-invariant component
is added. This becomes especially clear in Figure 4, where the point
estimate of the SGRI-CLPM is plotted against the point estimate of the
SRI-CLPM on the same data, which shows that these are virtually
identical.

\begin{figure}[H]

\caption{Interquartile ranges for estimated cross-lagged parameters per
model, per confounding condition. Dashed line represents the true value
of .1.}

{\centering \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{images/fig_srig_comparison.png}
...
\caption{Point estimates for both model specifications on the same data
plotted against one another, per confounding condition. Dashed line
represents equality between both estimates.}

{\centering \includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{images/fig_scatter.png}
...
Table x~shows the mean random intercept variances of \(X\) per
condition, alongside the mean point estimate of the cross-lagged
parameter of \(X\) on \(Y\). When the unconfounded CLPM generated the
data, the SGRI-CLPM correctly estimate random intercept variances around
0 for both the segmented and global random intercepts, but it does
suffer from improper solutions by estimating negative variances.
Interestingly, the global random intercept seems to be able to capture
the stable component relatively well. In the conditions where a
time-invariant confounder was present (time-invariant and combined) the
mean estimate is relatively close to the true value of 1. There does
...
\section{TAB2:SGRI-CLPM VARS}\label{tab2sgri-clpm-vars}
...
threat to the estimation of structural parameters in SEM Murayama \&
Gfrörer (\citeproc{ref-murayamaThinkingClearlyTimeinvariant2024}{2024}).
...
\bibitem[\citeproctext]{ref-lutkepohlNewIntroductionMultiple2007}
Lütkepohl, H. (2007). \emph{New introduction to multiple time series
analysis: With ... 36 tables} (1. ed., corr. 2. print). Berlin
Heidelberg: Springer.
