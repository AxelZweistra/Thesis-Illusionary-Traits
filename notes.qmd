...
# ==============================================================================
dgm_conditions <- list(
  list(name = "Pure-CLPM",    n_x_confounders = 0, n_y_confounders = 0, ri_var_x = 0, ri_var_y = 0, ri_cor = 0),
  list(name = "RI-CLPM",      n_x_confounders = 0, n_y_confounders = 0, ri_var_x = 1, ri_var_y = 1, ri_cor = 0.1),
  list(name = "Time-Varying", n_x_confounders = 5, n_y_confounders = 5, ri_var_x = 0, ri_var_y = 0, ri_cor = 0),
  list(name = "Combined",     n_x_confounders = 5, n_y_confounders = 5, ri_var_x = 1, ri_var_y = 1, ri_cor = 0.1)
)
...
extract_fit <- function(fit_obj, converged, param_name) {
  if (converged && param_name %in% names(coef(fit_obj))) coef(fit_obj)[param_name] else NA_real_
}

extract_se <- function(se_vec, param_name) {
  if (!is.null(se_vec) && param_name %in% names(se_vec)) se_vec[param_name] else NA_real_
}

# MEMORY OPTIMIZATION: Return a lightweight `list` instead of a `data.frame`
fit_model <- function(model_spec, data_df, model_label, dgm_label, rep_id, true_bx, true_by) {
  start_t <- Sys.time()
  
  fit <- tryCatch(sem(model = model_spec, data = data_df, estimator = "ML"), error = function(e) NULL)
  ...
  if (!is.null(fit)) {
    ...
    fit_meas <- tryCatch(
      ...
      error = function(e) setNames(rep(NA_real_, 9), c("chisq","df","pvalue","cfi","tli","rmsea","srmr","aic","bic"))
    )
  } else {
    converged  <- FALSE
    post_check <- FALSE
    ...
    coef_vec   <- NULL
    se_vec     <- NULL
    fit_meas   <- setNames(rep(NA_real_, 9), c("chisq","df","pvalue","cfi","tli","rmsea","srmr","aic","bic"))
  }
...
total_fits <- length(dgm_conditions) * n_replications * 4L
replication_results <- data.frame(
  replication      = integer(total_fits),
  dgm_condition    = character(total_fits),
  model_type       = character(total_fits),
  convergence      = logical(total_fits),
  post_check       = logical(total_fits),
  n_iterations     = integer(total_fits),
  bx               = numeric(total_fits),
  by               = numeric(total_fits),
  bx_se            = numeric(total_fits),
  by_se            = numeric(total_fits),
  bias_bx          = numeric(total_fits),
  bias_by          = numeric(total_fits),
  ax               = numeric(total_fits),
  ay               = numeric(total_fits),
  ax_se            = numeric(total_fits),
  ay_se            = numeric(total_fits),
  varRIx           = numeric(total_fits),
  varRIy           = numeric(total_fits),
  varRIx_se        = numeric(total_fits),
  varRIy_se        = numeric(total_fits),
  covRI            = numeric(total_fits),
  chisq            = numeric(total_fits),
  df               = numeric(total_fits),
  pvalue           = numeric(total_fits),
  cfi              = numeric(total_fits),
  tli              = numeric(total_fits),
  rmsea            = numeric(total_fits),
  srmr             = numeric(total_fits),
  aic              = numeric(total_fits),
  bic              = numeric(total_fits),
  processing_time  = numeric(total_fits),
  stringsAsFactors = FALSE
)
...
for (dgm in dgm_conditions) {
  ...
  for (rep in 1:n_replications) {
    ...
    # Insert list values directly into our pre-allocated dataframe
    replication_results[result_idx, ] <- fit_model(modelSpec_clpm, data$data, "CLPM", dgm$name, rep, true_bx, true_by)
    result_idx <- result_idx + 1L

    replication_results[result_idx, ] <- fit_model(modelSpec_riclpm, data$data, "RI-CLPM", dgm$name, rep, true_bx, true_by)
    result_idx <- result_idx + 1L

    replication_results[result_idx, ] <- fit_model(modelSpec_sri, data$data, "SRI-CLPM", dgm$name, rep, true_bx, true_by)
    result_idx <- result_idx + 1L

    replication_results[result_idx, ] <- fit_model(modelSpec_sri_global, data$data, "SRI-CLPM-Global", dgm$name, rep, true_bx, true_by)
    result_idx <- result_idx + 1L

    # Aggressive memory cleanup
    rm(data)
    if (rep %% 20 == 0) gc(verbose = FALSE) # Clear loose overhead frequently
  }
}
...
data_struct$results <- replication_results
data_struct$simulation_parameters <- list(
  n_replications   = n_replications, N_participants   = N_participants,
  timepoints       = timepoints, timespan         = timespan,
  across_seg_cov   = across_seg_cov, autocorr_level   = autocorr_level,
  beta_focal       = beta_focal, dgm_conditions   = dgm_conditions,
  true_bx          = true_bx, true_by          = true_by,
  true_ax          = true_ax, true_ay          = true_ay
)
...
sim     <- sim_dgm_comparison_T6_20260303_103953
results <- sim$results

true_beta <- 0.1

plot_data <- results %>%
  filter(convergence == TRUE, post_check == TRUE) %>%
  mutate(
    model_type    = factor(model_type, levels = c("CLPM", "RI-CLPM", "SRI-CLPM")),
    dgm_condition = factor(dgm_condition,
                           levels = c("Pure-CLPM", "RI-CLPM",
                                      "Time-Varying", "Combined"))
  )

dgm_levels <- levels(plot_data$dgm_condition)

# ==============================================================================
# PLOT 1 - Empirical Distribution of bx Estimates (one plot per DGM)
# ==============================================================================

p1_list <- lapply(dgm_levels, function(dgm) {
  ggplot(filter(plot_data, dgm_condition == dgm),
         aes(x = model_type, y = bx, fill = model_type)) +
    ...
    labs(
      title = paste0("Distribution of bx Estimates - ", dgm),
...
# ==============================================================================
# PLOT 2 - Distribution of Estimated Standard Errors for bx (one plot per DGM)
# ==============================================================================

p2_list <- lapply(dgm_levels, function(dgm) {
  ggplot(filter(plot_data, dgm_condition == dgm),
         aes(x = model_type, y = bx_se, fill = model_type)) +
    geom_boxplot(alpha = 0.7, outlier.shape = 21, outlier.alpha = 0.5) +
    scale_fill_brewer(palette = "Greens", name = "Model") +
    labs(
      title = paste0("Distribution of SE(bx) - ", dgm),
      y     = "Standard Error",
      x     = NULL
    ) +
    theme_minimal() +
...
# Print plots
for (p in p1_list) print(p)
for (p in p2_list) print(p)

# ==============================================================================
# TABLE 1 - Significance rates
# Note: p-values not saved in results; significance derived from 95% CI
# ==============================================================================

significance_table <- plot_data %>%
  mutate(
    sig_bx = (bx - 1.96 * bx_se) > 0 | (bx + 1.96 * bx_se) < 0,
    sig_by = (by - 1.96 * by_se) > 0 | (by + 1.96 * by_se) < 0
  ) %>%
  group_by(dgm_condition, model_type) %>%
  summarise(
    sig_bx_pct  = mean(sig_bx, na.rm = TRUE) * 100,
    sig_by_pct  = mean(sig_by, na.rm = TRUE) * 100,
    n_converged = n(),
    .groups     = "drop"
  )
...
kable(significance_table,
      digits    = 1,
      col.names = c("DGM", "Model", "Sig. bx (%)", "Sig. by (%)", "Converged N"),
      caption   = "Percentage of Significant Cross-Lagged Parameters (Alpha = .05, via 95% CI)")

# ==============================================================================
# TABLE 2 - SE Bias
# ==============================================================================

se_bias_analysis <- plot_data %>%
  group_by(dgm_condition, model_type) %>%
  summarise(
    Empirical_SE      = sd(bx, na.rm = TRUE),
    Mean_Estimated_SE = mean(bx_se, na.rm = TRUE),
    Bias_Percent      = ((Mean_Estimated_SE - Empirical_SE) / Empirical_SE) * 100,
    .groups           = "drop"
  )

kable(se_bias_analysis,
      digits    = 3,
      col.names = c("DGM", "Model", "Empirical SE", "Mean Estimated SE", "Bias (%)"),
      caption   = "SE Bias in bx Estimates")

# ==============================================================================
# TABLE 3 - Coverage rates
# ==============================================================================

coverage_analysis <- plot_data %>%
  mutate(
    ci_lower = bx - 1.96 * bx_se,
    ci_upper = bx + 1.96 * bx_se,
    covered  = true_beta >= ci_lower & true_beta <= ci_upper
  ) %>%
  group_by(dgm_condition, model_type) %>%
  summarise(
    coverage_rate = mean(covered, na.rm = TRUE) * 100,
...
kable(coverage_analysis,
      digits    = 1,
      col.names = c("DGM", "Model", "Coverage Rate (%)", "Valid Replications"),
...
# ==============================================================================
# HELPER: fit model and extract relevant estimates
# ==============================================================================

extract_estimates <- function(data, timepoints, timespan = 2) {
  fit <- sem(generate_segmented_riclpm_global(T = timepoints, timespan = timespan),
             data = data$data)
  est <- coef(fit)

  g <- function(nm) if (nm %in% names(est)) unname(est[nm]) else NA_real_
...
# ==============================================================================
# DATA GENERATION & FITTING
# ==============================================================================

data1 <- DGM(N = 100000, timepoints = timepoints,
             ri_var_x = 1, ri_var_y = 1, ri_cor = 0.1,
             n_x_confounders = 0, n_y_confounders = 0, seed = seed)

data2 <- DGM(N = 100000, timepoints = timepoints,
             n_x_confounders = 5, n_y_confounders = 5, seed = seed)

data3 <- DGM(N = 100000, timepoints = timepoints,
             ri_var_x = 1, ri_var_y = 1, ri_cor = 0.1,
             n_x_confounders = 5, n_y_confounders = 5, seed = seed)

e1 <- extract_estimates(data1, timepoints)
e2 <- extract_estimates(data2, timepoints)
e3 <- extract_estimates(data3, timepoints)
...
results_table <- data.frame(
  Parameter   = c("varRIgx", "varRIgy", "varRIx (segmented)",
                  "varRIy (segmented)", "covRIg"),
  True_C1     = c(1, 1, 0, 0, 0.1),
  Est_C1      = c(e1$varRIgx, e1$varRIgy, e1$varRIx, e1$varRIy, e1$covRIg),
  True_C2     = c(0, 0, 0, 0, 0),
  Est_C2      = c(e2$varRIgx, e2$varRIgy, e2$varRIx, e2$varRIy, e2$covRIg),
  True_C3     = c(1, 1, 0, 0, 0.1),
  Est_C3      = c(e3$varRIgx, e3$varRIgy, e3$varRIx, e3$varRIy, e3$covRIg)
)
...
kable(results_table,
      ...
      col.names = c("Parameter",
                    ...
                    "True", "Estimated"),
      caption   = paste0(
        "Population-level estimates (N = 100,000, T = ", timepoints,
        ", timespan = 2). ",
        "C1 = time-invariant only; C2 = time-varying only; C3 = both."
      ))
```

### Degrees of Freedom

Test: T=6, results in 49 DF, 59 model parameters
...
arr <- numeric(replications)

for (i in seq(1,replications)){
  
  df <- DGM(N = N, timepoints = timepoints,
            n_x_confounders = nconf, n_y_confounders = nconf,
            seed = i)$data

  fit <- sem(generate_segmented_riclpm(timepoints, timespan = 2), data = df)
  fit2 <- sem(generate_riclpm(timepoints), data = df)

  anv <- anova(fit,fit2)
  arr[i] <- anv$`Pr(>Chisq)`[2]
  
  if(i%%5 == 0){cat("At replication: ", i, " of ", replications)}
}
...
for (i in 1:replications) {
  df <- DGM(N = N, timepoints = timepoints, beta_xy = .1, beta_yx = 0, 
            seed = i)$data

  fit <- sem(generate_segmented_riclpm(T = 16, timespan = 2), data = df)
  coef <- parameterEstimates(fit)
  
  bx_row <- coef[coef$label == 'bx', ]
  by_row <- coef[coef$label == 'by', ]
  
  results_list[[i]] <- data.frame(
    rep = i,
    bx_est = bx_row$est[1],
    bx_p   = bx_row$pvalue[1],
    by_est = by_row$est[1],
    by_p   = by_row$pvalue[1]
  )
}
...
mean(final_results$bx_p<.05)
mean(final_results$by_p<.05)
```
