...
Kleine sanity check - waarom werken de SRI-CLPM's zo slecht bij T=6?
...
for (i in seq(1:200)){
  ...
  data <- DGM(N = 250, timepoints = 15, autocorr_effects = .3, ri_var_x = 1, ri_var_y = 1, ri_cor = .1, seed = i)
  
  fit <- sem(generate_riclpm(15), data$data)
  fit1 <- sem(generate_segmented_riclpm(15, timespan = 3), data$data)
  fit2 <- sem(generate_segmented_riclpm_global(15, timespan = 3), data$data)  
...
Takeaways from sanity checks:

On few timepoints (6), using a timespan of 2 seems to really overcorrect for the combined confounding (means around .93)

Increasing it to a timespan of 3 fixes the correction a lot, actually halves bias at the cost of a +- 20% increase in SE.

Checking that the time-varying stuff still works for the new DGM:

Increasing autocorr effects to .3 -\> makes the performance gap between RI-CLPM clearer, but not that much. Maybe its the T?

T = 15: gap between RI-CLPM and SRI-CLPM becomes much clearer.

Timepoints voor simulaties

T = 6, 12, 24, evt 48

N varieren van 150, 300?
